...
Run it - calling the program :file:`hexagon.py` - and check that it draws a hexagon in the turtle window.
...
#. The condition used after the ``while`` keyword is just like when using an ``if`` statement - a boolean expression.  It can use any of the comparison and logical operators.
