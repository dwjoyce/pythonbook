% Sans font
\usepackage{sans}
\renewcommand{\sfdefault}{\rmdefault}

% Start numbering at zero
\setcounter{chapter}{-1}
\renewcommand\thesection{\thechapter.\the\numexpr\value{section}-1\relax}

% Code block border
% \fvset{frame=lines}
\fvset{formatcom=\color{black!20}}


% Code background

\usepackage{newverbs}
\newcommand{\boxything}[1]{\raisebox{0pt}[6pt]{#1}}
\newcommand{\setlengths}{\setlength\parskip{0pt}\setlength\itemsep{0ex}\setlength\topsep{0ex}\setlength\partopsep{0pt}\setlength\leftmargin{0pt}}
\renewcommand{\code}[2][0pt]{{\setlengths\raisebox{#1}{\fcolorbox{VerbatimBorderColor}{VerbatimColor}{\boxything{\small\texttt{\color{black!20}#2}}}}}}

% \newcommand{\icode}{\collectverb{\icodebg}}

% Quote

\let\Oldquote\quote
\let\Oldendquote\endquote
\def\quote{\Oldquote\itshape}
\def\endquote{\upshape\Oldendquote}
